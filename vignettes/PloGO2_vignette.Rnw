...
res.list <- processAnnotation(file.names, GOIDlist, printFiles = TRUE)
@
...
res.list <- processAnnotation(file.names, GOIDlist, data.file.name = datafile, 
	printFiles = TRUE, datafile.ignore.cols = 2)
@
...
<<withAbundance,eval=TRUE, echo=TRUE>>=
writeAnnotation(res.list, datafile = datafile, format = "matrix")
@
...
<<processPathFile, eval=FALSE>>=
datafile <- paste(dir, "Abundance data.csv", sep="/")
...
plotAbundanceBar(res$aggAbundance, res$Counts)
@

\begin{figure}[h]
\includegraphics{Abundance barplot.png}
